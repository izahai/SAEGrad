...
try:
    from single_step_train.vis import LossCurveVisualizer
except ImportError:
    from vis import LossCurveVisualizer
...
def run_esd_training(config: ESDConfig) -> str:
    ...
    visualizer = LossCurveVisualizer(
        num_inference_steps=config.num_inference_steps,
        output_dir=os.path.join(config.save_path, "visualization"),
        file_prefix=(
            f"{adapter.family}_{sanitize_checkpoint_name(config.erase_concept)}"
            f"_from_{sanitize_checkpoint_name(config.erase_from_effective)}"
            f"_{config.train_method}"
        ),
    )
    ...
    for _ in pbar:
        ...
        loss.backward()
        optimizer.step()
        visualizer.record(loss.item(), step_result.timestep_index)
    ...
    visualizer.save_plot(title=f"ESD Loss Curve - {config.erase_concept}")
