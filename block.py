...
class IDMLP(nn.Module):
    def __init__(
        ...
        init: str = None,
        act: str = None,
        rank: int = None, # low-rank dim
        n_modes: int = None # ?
    ):
        super().__init__()
        LOG.info(f"Building IDMLP ({init}) {[indim] * (n_hidden + 2)}") # Building IDMLP (xavier) [256, 256, 256, 256] (layer sizes)
    ...
    def forward(self, x, mode=None): # mode ?
